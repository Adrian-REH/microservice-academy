package service.demo.store.shoppingservice.infrastructure.web;
...
import service.demo.store.shoppingservice.application.*;
...
public class InvoiceController {
    ...
    public ResponseEntity<List<Invoice>> listAllInvoices() {
        List<Invoice> invoices = allInvoice.execute();
        if (invoices.isEmpty()) {
    ...
    public ResponseEntity<Invoice> getInvoice(@PathVariable("id") String id) {
        ...
        Invoice invoice  = oneInvoice.execute(id);
        if (null == invoice) {
    ...
    public ResponseEntity<Invoice> createInvoice(@Valid @RequestBody Invoice invoice, BindingResult result) {
        ...
        Invoice invoiceDB = createInvoice.execute(invoice);
    ...
    // ------------------- Update a Invoice ------------------------------------------------
    @PutMapping(value = "/{id}")
    public ResponseEntity<?> updateInvoice(@PathVariable("id") String id, @RequestBody Invoice invoice) {
        log.info("Updating Invoice with id {}", id);

        invoice.setId(id);
        Invoice currentInvoice=updateInvoice.execute(invoice);
    ...
    public ResponseEntity<Invoice> deleteInvoice(@PathVariable("id") String id) {
        ...
        return ResponseEntity.ok(deleteInvoice.execute(id));
    }
